...
<div class="body" id="mk_height">
	  <table width="95%" border="0" align="center" cellpadding="5" cellspacing="0">
        <tr>
          <td class="header">Shoes Added By Guests
          &nbsp;&nbsp;&nbsp;&nbsp;
          <input type="button" name="button3" id="button3" value="          EXPORT TO EXCEL          " onclick="javascript: location.href='exporttoexcel.php?stat=12&d1=<?php echo $d1; ?>&d2=<?php echo $d2; ?>&period=<?php echo $period; ?>';" /></td>
        </tr>
        <tr>
...
<table width="95%" border="1" cellpadding="5" cellspacing="1" align="center" >
		  ...
		  if($period=='d')
			  $s="select count(*) as cnt,added from soe_shoe where 1=1 ".$cond."  and email!='' group by year(FROM_UNIXTIME(added)),month(FROM_UNIXTIME(added)),day(FROM_UNIXTIME(added)) order by cnt desc";  	

		  if($period=='w')
			  $s="select week(FROM_UNIXTIME(added)) as w,year(FROM_UNIXTIME(added)) as y,count(*) as cnt,added from soe_shoe where 1=1 ".$cond." and email!=''  group by year(FROM_UNIXTIME(added)),week(FROM_UNIXTIME(added)) order by cnt desc";  	

		  if($period=='m')
			  $s="select year(FROM_UNIXTIME(added)) as y,month(FROM_UNIXTIME(added)) as m,sto_id,count(*) as cnt,added from soe_shoe where 1=1 ".$cond." and email!=''  group by year(FROM_UNIXTIME(added)),month(FROM_UNIXTIME(added)) order by cnt desc";  	

		  if($period=='y')
			  $s="select year(FROM_UNIXTIME(added)) y,sto_id,count(*) as cnt,added from soe_shoe where 1=1 ".$cond." and email!=''  group by year(FROM_UNIXTIME(added)) order by cnt desc";  	
