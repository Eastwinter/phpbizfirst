...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	...
	if($_GET['id']>0)
	{
		$sql="update soe_members set first_name='".$_POST['first_name']."',last_name='".$_POST['last_name']."',dateofbirth='".$_POST['dateofbirth']."',gender='".$_POST['gender']."',email='".$_POST['email']."',assignedto=".$_POST['advt']." where mem_id=".$_GET['id'];
	...
	else
	{
            ...
            <script>
				...
				location.href="addadvstaff.php";
			</script>
            <?php
			die();
			}
	
		$sql="insert into soe_members set first_name='".$_POST['first_name']."',last_name='".$_POST['last_name']."',dateofbirth='".$_POST['dateofbirth']."',gender='".$_POST['gender']."',email='".$_POST['email']."',member_type='staff',password='".md5($_POST['password'])."',joined='".strtotime(date('Y-m-d'))."',assignedto=".$_POST['advt'].",verified='1',banned='0',reg='1'";
		mysql_query($sql) or die(mysql_error());
		$id=mysql_insert_id();
	...
	?>
    <script>
	location.href='showadvstaffers.php';
	</script>
    <?php
}
...
<div class="body" id="mk_height">
		...
		<Table width="95%" border="0" cellpadding="0" cellspacing="0" align="center">		
			<tr>
				<td class="header">Add / Edit I.C. Staffer</td>
			</tr>
			<tr>
				<td width="100%">					
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="5" align="ceneter" class="marginTop">
						  ...
						  <td><input type="text" name="last_name" value="<?php echo $last_name; ?>" class="input_box" id="last_name" maxlength="250" /></td>
							...
							<tr>
							  ...
							  <td><input type="text" name="email" value="<?php echo $email; ?>" class="input_box" id="email" maxlength="250" /></td>
							</tr>
							...
							<tr>
								<td class="tblcell"> Password</td>
							  <td><input type="password" name="password" class="input_box" id="password" maxlength="250" /></td>
							</tr>
							
							<tr>
								<td class="tblcell">Confirm  Password</td>
							  <td><input type="password" name="confirm_password" class="input_box" id="confirm_password" maxlength="250" /></td>
							</tr>
							
							<tr>
                              <td class="tblcell">Assigned To Advertiser*</td>
							  <td><select name="advt">
                              <option value="" selected="selected">None</option>
                              <?php
							  $rs=mysql_query("select * from soe_members where member_type='adv' order by first_name");
							  while($row=mysql_fetch_array($rs))
							  {
							  ?>
                                  <option value="<?php echo $row['mem_id']; ?>" <?php if($row['mem_id']==$assignedto) { ?> selected="selected" <?php } ?> >
