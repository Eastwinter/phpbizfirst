...
if(mysql_num_rows($rs)>0)
{
	$row=mysql_fetch_array($rs);
		...
		?>
			<script>
			location.href='advlogin.php?msg=alreadyregistered&id=<?php echo $id; ?>';
			</script>
        <?php
		...
		?>
			<script>
			location.href='advregister.php?msg=alreadyverified&id=<?php echo $id; ?>';
			</script>
        <?php
			}
		}
		...
		{			
			$sql="insert into soe_members set email='".$row['email']."',member_type='adv',from_ip='".$_SERVER['REMOTE_ADDR']."',joined='".time()."',verified='1'";
			$rs=mysql_query($sql);
			$id=mysql_insert_id();
			?>
			<script>
			location.href='advregister.php?msg=verify&id=<?php echo $id; ?>';
			</script>
			<?php
		}
}
...
?>
			<script>
			location.href='advlogin.php?msg=invalidcode&id=<?php echo $id; ?>';
			</script>
        <?php
}
?>	
