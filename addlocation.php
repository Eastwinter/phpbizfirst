...
`active` ='1',address='".$_POST['address']."',zip_code='".$_POST['zip_code']."',con_id='".$con_id."',sta_id='".$sta_id."',cty_id='".$_POST['cty_id']."',display_address='".$dispaddr."',`storeopen`='".mysql_real_escape_string($storeopen)."'  
 where loc_id=".$_GET['id'];
...
`transportation`='".mysql_real_escape_string($_POST['transportation'])."' ,
`directions` ='".mysql_real_escape_string($_POST['directions'])."',
`active` ='1',address='".$_POST['address']."',zip_code='".$_POST['zip_code']."',con_id='".$con_id."',sta_id='".$sta_id."',cty_id='".$_POST['cty_id']."',display_address='".$dispaddr."',added='".time()."',`storeopen`='".mysql_real_escape_string($storeopen)."'  
";
mysql_query($sql) or die(mysql_error());
...
$sql="insert into soe_membership set mem_id='".$_SESSION['memberid']."',sto_id=".$_GET['stoid'].",loc_id=".$locid.",packageid='".$row['packageid']."',price_exc_vat='".floatval($price_exc_vat)."',vat='".floatval($vat)."',total='".floatval($total)."',subscription='".$row['subscription']."'";
...
?>
        <script>
		location.href='paypal_loc.php?memshipid=<?php echo $memshipid; ?>';
		</script>
 <?php
...
?>
        <script>
		location.href='locations.php?id=<?php echo $_GET['stoid']; ?>';
		</script>
 <?php
 }
}
...
jQuery(function() {
  ...
  <div id="content_area_mid_inner1">
    <div class="border1">
    <div>
    <h2>My Stores - Add New Locations</h2>
  </div>
...
<br class="clear">
      ...
      <?php
        ...
        <input autocomplete="off" name="city" type="text" class="txt_box1" size="35" id="city" value="<?php echo $cityname; ?>">
            ...
            <br class="clear" />
      <div class="input_title"><label for="cross_street">Cross Street</label></div>
			<div class="input">
			  <input name="cross_street" type="text" id="cross_street" value="<?php echo $cross_street; ?>" size="40" />
			</div>
			<br class="clear">
			
			<div class="input_title"><label for="transportation">Public Transportation</label></div>
			<div class="input">
			  <textarea name="transportation" cols="40" rows="4" id="transportation"><?php echo $transportation; ?></textarea>
			</div>
			<br class="clear">
			
			<div class="input_title"><label for="directions">Directions</label></div>
			<div class="input">
			  <textarea name="directions" cols="40" rows="4" id="directions"><?php echo $directions; ?></textarea>
			</div>
			<br class="clear">
	  <div class="input_title"><label for="hour_type">Hours of Operation</label></div>
		
               <input name="hour_type" value="0" type="radio" <?php if($hour_type==0) { ?> checked="checked" <?php } ?> >Do not Display Operation Hours<br>
				<input name="hour_type" value="1" type="radio" <?php if($hour_type==1) { ?> checked="checked" <?php } ?> >24 Hours a Day<br>
				<input name="hour_type" value="2" type="radio" <?php if($hour_type==2) { ?> checked="checked" <?php } ?> >Use Hours of Operation Below<br>
			...
			<div class="input"><input name="submit" value="Submit" class="bttn" type="submit"></div>
			<br class="clear">
